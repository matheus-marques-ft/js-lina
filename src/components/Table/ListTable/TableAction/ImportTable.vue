...
export default {
  ...
  data() {
    return {
      ...
      defaultTableConfig: {
        ...
        paginationSizes: [10],
        tableAttrs: {
          stripe: true, // striped table
          border: true, // table border
          fit: true, // width auto-fit,
          // Cell overflow tooltip has limited width and breaks words, avoiding overly long content (e.g. ciphertext) from stretching the page horizontally.
          // popperStyle is passed directly to the popper generated by Element Plus; relying only on the CSS class name isn't enough.
          tooltipOptions: {
            effect: 'dark',
  ...
  methods: {
      ...
      for (const item of tableTitles) {
        const dataItemLens = tableData.map((d) => {
          ...
          if (typeof itemColData === 'boolean') {
            return 5 // boolean is 5 characters long and boolean.length is undefined
    ...
    async performUpload() {
      ...
      // Without affecting other components that use this component, handle the import button change in cloud sync
...
// Status tab / progress bar / table / buttons all use flex column layout, with consistent spacing
.import-table-body {
  display: flex;
...
// Top status tab uses a uniform 30px height
...
// Bottom buttons follow the Dialog footer convention: divider + right-aligned + 12px gap + default size
.import-footer {
  display: flex;
  ...
  // The button is inside the dialog body and doesn't inherit the site-wide .dialog-footer convention, so align it here to the same 30px height
  :deep(.el-button) {
    min-height: 30px;
    height: 30px;
    padding: 8px 12px;
...
<!-- The table cell overflow tooltip teleports to body, so its width must be constrained via non-scoped styles -->
<style lang="scss">
